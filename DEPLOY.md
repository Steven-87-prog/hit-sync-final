...
## Optional: real SMS notifications (Twilio)

By default, hit requests/accepts/declines/cancellations show up as in-app
notifications only. If you want them to also text a player's phone (when
they've added one on Profile), you'll need a **Twilio** account — this is a
real paid service, not free, so this section is entirely optional and the
app works completely fine without it.

**Cost:** roughly $1/month to rent a phone number, plus about $0.0079 per
text sent (US pricing as of when this was written — check
[twilio.com/pricing](https://www.twilio.com/en-us/pricing) for current
rates). Twilio also offers a free trial with a small credit to test with
before you pay anything.

1. Go to [twilio.com](https://www.twilio.com) → sign up.
2. From the [Twilio Console](https://console.twilio.com), copy your
   **Account SID** and **Auth Token** (shown right on the dashboard homepage).
3. Buy a phone number: **Phone Numbers → Buy a number** → pick any number
   with SMS capability (US numbers are fine for texting US phones).
4. On your host, add three environment variables:
   - `TWILIO_ACCOUNT_SID` — from step 2
   - `TWILIO_AUTH_TOKEN` — from step 2
   - `TWILIO_FROM_NUMBER` — the number you bought in step 3, in `+1XXXXXXXXXX` format
5. Redeploy (or it'll pick these up automatically on the next deploy).

That's it — no code changes needed. The app checks for these three
variables at startup; if they're all present, SMS sending turns on
automatically. If any are missing, it silently skips SMS and only sends the
in-app notification, so there's no risk of breaking anything by leaving
this unconfigured.

**Trial account limitation:** while on Twilio's free trial (before you add
a payment method), it can only text phone numbers you've manually verified
in the Twilio Console — it can't text arbitrary players yet. This is a
Twilio restriction, not something in the app. Add a payment method when
you're ready to text real, unverified numbers.
