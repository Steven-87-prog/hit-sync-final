...
// Maps a YYYY-MM-DD calendar date to the 3-letter day abbreviation used
// throughout availability blocks ("Mon".."Sun") — matches purely on the
// calendar date itself, not a specific instant, so this is timezone-safe.
function dayAbbrevFromDate(dateStr) {
  const ABBR = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
  const d = new Date(`${dateStr}T00:00:00`);
  return ABBR[d.getDay()];
}

// Once a hit is confirmed, the person is no longer "available" during that
// exact slot — this trims their saved availability so it reflects reality.
// Only handles one-time, exact-date blocks (the default block type going
// forward); a legacy recurring block is left alone since removing it would
// wrongly wipe out every future week, not just this one occurrence.
function trimAvailabilityForHit(user, hit) {
  const hitDay = dayAbbrevFromDate(hit.date);
  const kept = [];
  for (const block of user.availability || []) {
    const isOneTime = block.recurring === false;
    const matchesThisHit = isOneTime && block.date === hit.date && block.day === hitDay;
    const overlapsTime = matchesThisHit && block.start < hit.endTime && block.end > hit.startTime;
    if (!overlapsTime) {
      kept.push(block);
      continue;
    }
    // Keep whatever portion of the block falls outside the booked window —
    // e.g. available 4-8pm, hit booked 5-6pm, leaves 4-5pm and 6-8pm.
    if (block.start < hit.startTime) kept.push({ ...block, end: hit.startTime });
    if (block.end > hit.endTime) kept.push({ ...block, start: hit.endTime });
    // Otherwise the hit fully covers the block — drop it entirely.
  }
  user.availability = kept;
}

// A one-time availability block whose date has already passed no longer
// represents real availability — this filters those out, using the given
// user's own timezone (not the server's) so "today" is accurate for them.
function currentAvailability(user) {
  const today = todayISO(user.timezone);
  return (user.availability || []).filter(
...
function publicUser(user) {
  ...
  const availability = currentAvailability(user);
