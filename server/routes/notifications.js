...
router.get("/", requireAuth, (req, res) => {
  const data = load();
  // Read notifications older than a week quietly clean themselves up —
  // unread ones stick around regardless of age so nothing gets missed.
  const oneWeekAgo = Date.now() - 7 * 24 * 60 * 60 * 1000;
  const before = data.notifications.length;
  data.notifications = data.notifications.filter(
    (n) => !(n.read && new Date(n.createdAt).getTime() < oneWeekAgo)
  );
  if (data.notifications.length !== before) save(data);
