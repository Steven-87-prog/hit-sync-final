...
const { load, save, nextId, publicUser, addNotification, sendSMS, withAppLink, todayISO, getRatingSummary } = require("../utils");
...
router.get("/calendar", requireAuth, (req, res) => {
  ...
  const mine = data.hits.filter(
    (h) => (h.fromId === req.userId || h.toIds.includes(req.userId)) && h.status === "accepted" && h.date >= today
  );
