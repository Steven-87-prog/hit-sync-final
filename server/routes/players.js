...
const { load, publicUser, userDistance, overlappingBlocks, getRatingSummary } = require("../utils");
...
router.get("/find", requireAuth, (req, res) => {
  ...
  if (availabilityMode === "set") candidates = candidates.filter((u) => (u.availability || []).length > 0);
