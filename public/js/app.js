...
function maybeShowDailyReminder() {
  ...
  showModal(`
    <div class="card" style="max-width:380px;margin:0 auto">
      ...
      <p style="color:var(--ink-soft);font-size:14px">
        Keep your profile up to date — especially your <strong>UTR</strong> — so other players see an accurate
        match. Also worth double-checking your availability is current.
      </p>
      <div style="display:flex;gap:6px;margin-top:16px">
        <button class="btn btn-ghost" id="reminder-avail" style="font-size:12px;padding:6px 10px;flex:1">Availability</button>
        <button class="btn btn-primary" id="reminder-profile" style="font-size:12px;padding:6px 10px;flex:1">Profile</button>
        <button class="btn btn-ghost" id="reminder-dismiss" style="font-size:12px;padding:6px 10px;flex:0 0 auto">Maybe later</button>
      </div>
    </div>`);
...
async function renderCalendar() {
  ...
  pageContent().innerHTML = `
    <div class="page-header">
      <span class="eyebrow">Confirmed hits</span>
      <h1>${firstDay.toLocaleDateString(undefined, { month: "long", year: "numeric" })}</h1>
      <p>Accepted hits appear on the full monthly calendar.</p>
    </div>
    ...
    <div class="card">
      ...
      <form id="profile-form" class="form-grid">
        ...
        <p class="full" style="font-size:11.5px;color:var(--ink-soft);margin:-6px 0 0">
          <strong>Location:</strong> ${user.lat != null ? `On${user.location ? ` — ${escapeHtml(user.location)}` : ""} <span style="font-weight:600">(${user.locationSource === "gps" ? "Precise — tracked live" : "Approximate — IP-based"})</span>` : "Off"} — used to calculate distance to other players. This is protected — never shown to anyone, only the distance in miles.
        </p>
